...
def create_page_object(page: CrawlPage) -> PageObject:
    ...
    for form_name, form_elements in page_object.forms.items():
        ...
        if submit_element and field_assignments:
            ...
            page_object.methods[f"fill_{form_name}"] = """async def fill_{form_name}(self, data: dict) -> None:
...
def generate_page_object_file(page_object: PageObject, output_dir: str) -> GeneratedFile:
    """
...
def create_test_from_page_object(page_object: PageObject) -> GeneratedTest:
    ...
    test.imports.add(f"from page_objects.{os.path.splitext(page_object.file_name)[0]} import {page_object.name}")
...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == "call" and report.failed:
        try:
            ...
            trace_path = os.path.join(trace_dir, f"{{item.nodeid.replace('/', '_')}}.zip")
